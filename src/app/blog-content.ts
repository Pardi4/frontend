...
const TRANSLATED_ARTICLE_DRAFTS: Record<string, ArticleDraft> = {
  'universal-parser-infinite-quiz-platforms': {
    intro: ['When we built QuizSolver, we started by tuning it perfectly for the biggest platforms: Testportal, Canvas, Moodle, and Google Forms. But the internet is full of custom quiz engines, HR assessments, and niche learning portals.', 'That\'s why we developed the Universal Parser™ — a smart layout engine that reads the DOM and figures out where the question and answers are, no matter the platform.'],
    sections: [
      { heading: 'How it works', paragraphs: ['Universal Parser™ scans the current page for common quiz structures: large blocks of text followed by radio buttons, checkboxes, or distinct clickable elements. It extracts the text cleanly and feeds it to our AI.'] },
      { heading: 'When to use FocusScan', paragraphs: ['If a quiz uses a completely locked visual interface, like a PDF embed or an HTML5 Canvas where the text isn\'t in the DOM, Universal Parser™ might not see it. In those cases, you can use our built-in FocusScan tool to crop the question manually.'] }
    ]
  },
  'universal-parser-nieskonczonosc-platform-quizowych': {
    intro: ['Kiedy tworzyliśmy QuizSolver, zaczęliśmy od perfekcyjnego dostosowania go do największych platform: Testportal, Canvas, Moodle i Google Forms. Jednak internet jest pełen autorskich silników quizowych, testów rekrutacyjnych i niszowych portali edukacyjnych.', 'Dlatego stworzyliśmy Universal Parser™ – inteligentny silnik, który analizuje strukturę strony i samodzielnie odnajduje pytania oraz odpowiedzi, niezależnie od platformy.'],
    sections: [
      { heading: 'Jak to działa', paragraphs: ['Universal Parser™ skanuje stronę w poszukiwaniu typowych struktur quizów: dużych bloków tekstu z przyciskami opcji (radio), polami wyboru lub wyraźnymi elementami klikalnymi. Czysty tekst trafia prosto do naszego AI.'] },
      { heading: 'Kiedy użyć FocusScan', paragraphs: ['Jeśli quiz używa zablokowanego interfejsu graficznego, np. osadzonego PDF-a lub canvasu HTML5, Universal Parser™ może go nie zobaczyć. W takich sytuacjach użyj narzędzia FocusScan, by ręcznie wyciąć obszar pytania z ekranu.'] }
    ]
  },
  'universal-parser-unendlich-viele-quiz-plattformen': {
    intro: ['Als wir QuizSolver entwickelten, begannen wir mit der perfekten Abstimmung auf die größten Plattformen: Testportal, Canvas, Moodle und Google Forms. Aber das Internet ist voller eigener Quiz-Engines und Nischen-Lernportale.', 'Deshalb haben wir den Universal Parser™ entwickelt – eine intelligente Engine, die die Struktur der Seite analysiert und Fragen sowie Antworten unabhängig von der Plattform findet.'],
    sections: [
      { heading: 'Wie es funktioniert', paragraphs: ['Der Universal Parser™ sucht auf der Seite nach typischen Quizstrukturen: große Textblöcke gefolgt von Auswahlfeldern oder klickbaren Elementen. Der saubere Text wird dann an unsere KI weitergeleitet.'] },
      { heading: 'Wann FocusScan genutzt wird', paragraphs: ['Wenn ein Quiz eine blockierte grafische Benutzeroberfläche nutzt, wie z.B. ein PDF oder HTML5-Canvas, sieht der Universal Parser™ den Text möglicherweise nicht. Dann hilft das FocusScan-Tool, mit dem der Fragebereich manuell markiert wird.'] }
    ]
  },
  'universal-parser-infinitas-plataformas-quizzes': {
    intro: ['Cuando creamos QuizSolver, empezamos afinándolo para las mayores plataformas: Testportal, Canvas, Moodle y Google Forms. Pero internet está lleno de motores de quizzes personalizados y portales de aprendizaje nicho.', 'Por eso desarrollamos Universal Parser™: un motor inteligente que analiza la estructura de la página y encuentra preguntas y respuestas, sin importar la plataforma.'],
    sections: [
      { heading: 'Cómo funciona', paragraphs: ['Universal Parser™ escanea la página buscando estructuras comunes de quizzes: bloques de texto grandes seguidos de botones de radio o casillas de verificación. Luego, envía el texto limpio a nuestra IA.'] },
      { heading: 'Cuándo usar FocusScan', paragraphs: ['Si un quiz usa una interfaz visual bloqueada, como un PDF o un Canvas HTML5 donde el texto no está en el DOM, Universal Parser™ podría no verlo. En esos casos, usa la herramienta FocusScan para seleccionar la pregunta manualmente.'] }
    ]
  },
  'universal-parser-infinite-plateformes-quiz': {
    intro: ['Lorsque nous avons créé QuizSolver, nous avons commencé par l\'optimiser pour les plus grandes plateformes : Testportal, Canvas, Moodle et Google Forms. Mais Internet regorge de moteurs de quiz personnalisés.', 'C\'est pourquoi nous avons développé Universal Parser™ : un moteur intelligent qui analyse la structure de la page et trouve les questions et les réponses, quelle que soit la plateforme.'],
    sections: [
      { heading: 'Comment ça marche', paragraphs: ['Universal Parser™ scanne la page à la recherche de structures de quiz courantes : de gros blocs de texte suivis de boutons radio ou de cases à cocher. Le texte est ensuite envoyé à notre IA.'] },
      { heading: 'Quand utiliser FocusScan', paragraphs: ['Si un quiz utilise une interface visuelle verrouillée, comme un PDF ou un Canvas HTML5, Universal Parser™ peut ne pas le voir. Dans ces cas, utilisez l\'outil FocusScan pour sélectionner la question manuellement.'] }
    ]
  },
  'universal-parser-infinite-piattaforme-quiz': {
    intro: ['Quando abbiamo creato QuizSolver, abbiamo iniziato ottimizzandolo per le piattaforme più grandi: Testportal, Canvas, Moodle e Google Forms. Ma internet è pieno di motori di quiz personalizzati e portali di nicchia.', 'Per questo abbiamo sviluppato Universal Parser™: un motore intelligente che analizza la struttura della pagina e trova domande e risposte, indipendentemente dalla piattaforma.'],
    sections: [
      { heading: 'Come funziona', paragraphs: ['Universal Parser™ scansiona la pagina alla ricerca di strutture di quiz comuni: grandi blocchi di testo seguiti da pulsanti di opzione o caselle di controllo. Il testo pulito viene poi inviato alla nostra IA.'] },
      { heading: 'Quando usare FocusScan', paragraphs: ['Se un quiz usa un\'interfaccia visiva bloccata, come un PDF o un HTML5 Canvas, Universal Parser™ potrebbe non vederlo. In questi casi, usa lo strumento FocusScan per selezionare manualmente la domanda.'] }
    ]
  },
  'universal-parser-bezlich-quiz-platform': {
    intro: ['Створюючи QuizSolver, ми почали з оптимізації для найбільших платформ: Testportal, Canvas, Moodle та Google Forms. Але в інтернеті повно нестандартних квіз-движків і нішевих порталів.', 'Тому ми розробили Universal Parser™ — розумний рушій, який аналізує структуру сторінки і знаходить питання та відповіді незалежно від платформи.'],
    sections: [
      { heading: 'Як це працює', paragraphs: ['Universal Parser™ сканує сторінку на наявність типових структур квізів: великих блоків тексту з радіокнопками, чекбоксами або клікабельними елементами. Потім чистий текст передається нашому AI.'] },
      { heading: 'Коли використовувати FocusScan', paragraphs: ['Якщо квіз використовує заблокований візуальний інтерфейс, як-от PDF або HTML5 Canvas, де текст відсутній у DOM, Universal Parser™ може його не побачити. У таких випадках скористайтеся інструментом FocusScan для ручного виділення питання.'] }
    ]
  },
