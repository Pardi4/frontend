...
const en: SiteCopy = {
  ...
  platformPages: {
    ...
    kahoot: {
      ...
      title: "Kahoot AI Bot — How to get answers instantly",
      ...
      meta: { title: "How to get Kahoot answers (AI Bot & Quiz ID) | QuizSolver", description: "Kahoot AI bot Chrome extension. Auto-solve visible Kahoot questions OR use Quiz ID to view the full answer bank — no credits needed. Works on all Kahoot game formats. Free to install." },
      ...
      keywordSections: [
        { title: "QuizSolver vs Kahoot Auto Bots", text: "Why do most Kahoot bots fail? Many websites promising 'Kahoot hacks' require you to inject spammy scripts or enter game PINs that get blocked by teachers. QuizSolver is safe: it runs locally in your Chrome browser side panel, reading questions and suggesting answers without triggering anti-cheat detectors." },
...
const pl: SiteCopy = {
  ...
  platformPages: {
    ...
    kahoot: {
      ...
      title: "Kahoot AI Bot — Jak szybko znaleźć odpowiedzi",
      ...
      meta: { title: "Kahoot AI Bot & Hack do Odpowiedzi (Quiz ID) | QuizSolver", description: "Rozszerzenie Chrome dla Kahoot. Automatyczne odpowiedzi AI na widoczne pytania LUB hack z Quiz ID — bez kredytów. Obejście Kahoot bezpieczne od wykrycia." },
      ...
      keywordSections: [
        { title: "QuizSolver vs Zwykłe Kahoot Boty", text: "Dlaczego większość botów do Kahoota nie działa? Większość stron obiecujących 'Kahoot hacks' każe wpisywać PIN gry i jest natychmiast blokowana przez nauczycieli. QuizSolver działa inaczej — w bezpiecznej piaskownicy przeglądarki Chrome, czytając pytania bez ingerencji w kod gry, dzięki czemu omija systemy anty-cheat." },
...
const LOCALIZED_SITE: Record<ExtraLocale, LocalizedSiteBundle> = {
  de: {
    ...
    privacyPage: { metaTitle: 'Datenschutz | QuizSolver', metaDescription: 'Erfahre, welche Konto-, Credit-, Quiz- und FocusScan-Daten QuizSolver verarbeitet und wie du Auskunft, Export oder Loeschung anfordern kannst.', title: 'Datenschutz', badge: 'Datenschutz & Sicherheit', subtitle: 'Wir schützen deine Daten und respektieren deine Privatsphäre.', effective: 'Gültig ab: 21. Mai 2026', contactLabel: 'Support kontaktieren', contactValue: 'support@getquizsolver.com', sections: [{ title: '1. Datenerfassung und Kontrolle', text: 'Wir erfassen nur Daten, die für Konto, Credits und den QuizSolver-Dienst nötig sind.', items: ['Kontodaten: E-Mail, Anzeigename und verschlüsseltes Passwort. Wir nutzen 200%ige Sicherheitsstandards (bcrypt 12 Runden). Ihre Passwörter sind sicher und unlesbar.', 'Nutzungsdaten: gelöste Fragen, Credits und Käufe.', 'Empfehlungen: Registrierungen und Käufe über deinen Code.'] }, { title: '2. Chrome-Erweiterung und FocusScan', text: 'Die Erweiterung analysiert nur Inhalte, wenn du sie aktiv auslöst.', items: ['Aktives Scannen: sichtbare Fragen und Antwortoptionen.', 'FocusScan: der von dir markierte Bildschirmausschnitt.', 'FocusScan-Bilder werden temporär von externen KI-Anbietern verarbeitet. Erfassen Sie keine sensiblen Daten.', 'Kein Tracking anderer Tabs oder des Browserverlaufs.'] }, { title: '3. Zahlungen', text: 'Zahlungen werden sicher über Lemon Squeezy verarbeitet. Wir speichern keine Kartennummern.' }, { title: '4. Weitergabe', text: 'Wir verkaufen oder vermieten keine persönlichen Daten.' }, { title: '5. Deine Rechte', text: 'Du kannst deine Daten einsehen, ändern oder löschen lassen.', items: ['Export deiner gespeicherten Fragen.', 'Kontolöschung über Einstellungen oder Support.'] }] },
  ...
  const templates: Record<Locale, {
    ...
    fr: {
      ...
      genericFaq: (name) => [
        ...
        { question: 'Et si la question n’est pas détectée ?', answer: 'Si la plateforme a changé son interface, utilisez FocusScan et encadrez la question avec les réponses.' },
    ...
    it: {
      ...
      genericFaq: (name) => [
        ...
        { question: 'E se non rileva la domanda?', answer: 'Se la piattaforma cambia layout, usa FocusScan e seleziona l’area con domanda e risposte.' },
...
function appendUniqueByTitle<T extends { title: string }>(items: T[] | undefined, additions: T[]): T[] {
  const existing = new Set((items || []).map(item => item.title));
  return [
    ...(items || []),
    ...additions.filter(item => !existing.has(item.title))
  ];
}

function appendUniqueByQuestion<T extends { question: string }>(items: T[] | undefined, additions: T[]): T[] {
  const existing = new Set((items || []).map(item => item.question));
  return [
    ...(items || []),
    ...additions.filter(item => !existing.has(item.question))
  ];
}

function enhancePriorityPlatformSeo(copy: SiteCopy, locale: Locale): SiteCopy {
  const isPl = locale === 'pl';
  const pages = copy.platformPages;
  const priorityContent: Partial<Record<keyof SiteCopy['platformPages'], {
    metaDescription: string;
  ...
  (Object.entries(priorityContent) as Array<[keyof SiteCopy['platformPages'], NonNullable<typeof priorityContent[keyof SiteCopy['platformPages']]>]>).forEach(([key, addition]) => {
...
enhancePriorityPlatformSeo(en, 'en');
enhancePriorityPlatformSeo(pl, 'pl');
